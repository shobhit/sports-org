...
try:
    ...
    if sys.version_info < (2, 5, 0):
        ...
        module = 'sqlite3'
    raise EnvironmentError, "Error loading %s module: %s" % (module, e)
...
"""
    ...
    def __init__(self, location = ':memory:'):
        ...
        self.intro = self.intro % (__version__,
                Database.sqlite_version, Database.version, location)

        self.db = Database.connect(location)
    ...
    use 'example.db'
        ...
        self.db.commit()
        self.db.close()
        self.db = Database.connect(dbase)
        self.db.isolation_level = None
        self.cur = self.db.cursor()
        print '\n Now using database \'%s\'\n' % dbase
        return
    ...
    def default(self, arg):
        ...
        if Database.complete_statement(query):
            try:
            ...
            while not Database.complete_statement(query):
